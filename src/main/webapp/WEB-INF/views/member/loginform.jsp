...
function checkValue(){
	...
	inputId = document.getElementById("m_id").value;
	inputPw = document.getElementById("m_pw").value;
	if(!inputId){ //id를 입력하지않으면
		alert("아이디입력없음");
		return false; //flase리턴되면 submit작동안함
	}
...
<h3>loginform.jsp</h3>
<h1>로그인화면</h1>
<form action="login" method="post" name="logininfo" onsubmit="return checkValue()"> <!-- submit 실행 전에 먼저실행됨 (flase리턴되면 submit작동안함) -->
